...
|---|---|---|---|---|---|
| A1 | **Orphaned subprocess tree on app exit.** No `CloseRequested`/`ExitRequested` handler, no `Drop` on `RunState`, no Windows Job Object — closing the window mid-run leaves the `node`→`openclaw`→`claude`/`git` tree running detached, **still spending API tokens**, with no UI to find or stop it. Only `cancel_run` kills the child. | HIGH | M | `lib.rs` run() builder (~2477) + `RunState` | Add a window `CloseRequested` handler (and/or `Drop`) that `start_kill()`s the child, ideally inside a Windows **Job Object** so grandchildren die with it. |
...
- **D3 — `--run-id` works.** The GUI's run UUID flows through to the record, the soft-cancel marker, and the event stream. (`--resume`'s load-prior-results/skip-succeeded-phases behavior remains **intentionally unimplemented** and is documented as such; `--group` stays a hint.)
