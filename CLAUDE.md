...
- **Completion tombstone (`.done`) is the watchdog's backstop, not the engine's.** The Rust done-handler writes `runs/<group-id>/<runId>.done` (`{exitCode,waitError}`, via `atomic_write`) when the orchestrator *process* exits — written by the **GUI shell**, unlike `.cancel`/`.json` which the Node engine owns, so the orchestrator never reads it. It exists only so the GUI watchdog (`run_done_status`, polled every 3 s) can un-stick the UI if the live `orchestrator:done` event is dropped. The GUI clears it on finish (`clear_run_done`) and sweeps orphans at startup (`sweep_done_tombstones`); `finishRun` is idempotent so the watchdog and a late live event can't double-finalize.
- **`--group <id>` is a one-shot override** (`src/cli.ts` → `loadConfig(overrideGroupId)`). It selects that group for the single CLI run without persisting; the GUI/on-disk `activeGroupId` is unchanged. (Persistent switching is still done in the GUI.)
