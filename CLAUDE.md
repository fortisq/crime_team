...
The GUI captures the answer by scraping the orchestrator's stdout between the line `PRODUCER'S INTEGRATED ANSWER` and `[ ok  ] done.` (`desktop/src/main.js`, `ANSWER_HEADER`). The Coder report is printed with its own banner so it lands inside that window too. If you change those log strings in `orchestrator.ts`, update the scraper.
